...
// Consumed by the backend at runtime (backend/src/routes/populationContext.ts)
// -- a static, derived artifact. Regenerated by this script, never hand-edited.
const populationContextOutPath = path.resolve(
  import.meta.dirname,
  "../../backend/src/data/nhanesPopulationContext.json"
)
const POPULATION_CONTEXT_BAND_WIDTH = 5
const POPULATION_CONTEXT_MIN_AGE = 18
const POPULATION_CONTEXT_MAX_AGE = 59
...
`
  ...
  // Population-context artifact for the running app (backend/src/routes/
  // populationContext.ts). Uses all included samples, not just train/val/test
  // -- this is population-level reporting, not model evaluation, so there's
  // no train/test leakage concern.
  const ageBands = binByAgeBand(
    samples,
    POPULATION_CONTEXT_BAND_WIDTH,
    POPULATION_CONTEXT_MIN_AGE,
    POPULATION_CONTEXT_MAX_AGE
  )
  const populationContext = {
    source: "NHANES 2017-2018 national survey (RHQ_J + DEMO_J)",
    generatedAt: timestamp,
    disclaimer:
      "Population-level statistics from a real national survey -- not clinical validation, not a diagnosis, and not a prediction about any individual.",
    ageBands
  }
  fs.mkdirSync(path.dirname(populationContextOutPath), { recursive: true })
  fs.writeFileSync(populationContextOutPath, JSON.stringify(populationContext, null, 2))
  console.log(`Wrote ${populationContextOutPath}`)
}
